...
import phyloselect.seq_mining.build_trimed as build_trimed
import phyloselect.seq_mining.fix_alignment as fix_alignment
import phyloselect.seq_mining.muscle_wrapper as muscle_wrapper
...
def do_filter_assemble(args, samples, do_filter, do_assemble, ignore_hook=lambda *_, **__: None):
    ...
    if do_filter:
        ...
        def run_filter(name):
            ...
            #
            #     for phyloselect in genes:
            #         read_1 = os.path.join(out_dir, f'{phyloselect}_1{sample_ext}')
            #         read_2 = os.path.join(out_dir, f'{phyloselect}_2{sample_ext}')
            #
            #         if not os.path.isfile(read_1):
            #             continue
            #
            #         with open(os.path.join(merge_dir, phyloselect + sample_ext), 'wb') as f:
...
def build_coalescent_tree(args):
    ...
    if not genes:
        raise RuntimeError(f"No phyloselect alignments found under '{alignment_dir}'")
    ...
    if not written:
        raise RuntimeError(f"Unable to reconstruct coalescent trees because no phyloselect tree is available")
...
def cli(argv=None):
    ...
    parser.add_argument('-m', '--tree-method', choices=('coalescent', 'concatenation'), default='coalescent', help='Multi-phyloselect tree reconstruction method (default = coalescent)')
